...
module.exports.createPost = (req,res)=>{
    ...
    if(Object.keys(files).length === 0){
        ...
        if(ext !== 'jpg' && ext !== 'jpeg' && ext !=='png'){
            errors.push({msg:`${ext} is not a valid extension`});
        }
        else{
...
module.exports.updateImage = (req,res)=>{
    ...
    form.parse(req,(error,fields,files)=>{
        ...
        if(Object.keys(files).length === 0){
            ...
            const ext = split[1].toLowerCase();
            if(ext!=='jpg' && ext!=='jpeg'&& ext!=='png'){
                imageErrors.push({msg:`${ext} is not a valied extension`})
            }else{
                files.image.name = uuidv4()+'.'+ ext;
            }
        }
...
module.exports.postComment = async(req,res)=>{
    ...
    try {
        ...
        return res.status(200).json({msg:'Your Comment is published'});
    } catch (error) {
        return res.status(500).json({errors:error,msg:error.message});
    }
}